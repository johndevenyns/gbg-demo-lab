        ...
        <iframe
          ...
          onLoad={(e) => {
            ...
            const parseCssColor = (color?: string | null) => {
              ...
              const value = namedColors[normalized] || normalized;
              ...
              if (rgbMatch) {
                ...
                const toChannel = (part: string) => {
                  ...
                  if (trimmed.endsWith('%')) {
                    return Math.max(0, Math.min(255, Math.round((parseFloat(trimmed) / 100) * 255)));
                  }
            ...
            const getReadableColor = (textColor?: string | null, bgColor?: string | null) => {
              ...
              if (textLuminance == null) return fallback;

              return Math.abs(textLuminance - backgroundLuminance) < 0.3 ? fallback : (textColor?.trim() || fallback);
            };
              ...
              try {
                ...
                if (doc && win && body) {
                  ...
                  const candidates = Array.from(
                    body.querySelectorAll<HTMLElement>('a, button, span, p, strong, em, small, label, li, h1, h2, h3, h4, h5, h6, div')
                  );

                  candidates.forEach((candidate) => {
                    if (!hasDirectText(candidate)) return;
                    ...
                    const readableColor = getReadableColor(computed.color, background);

                    if (readableColor !== computed.color) {
                      target.style.setProperty('color', readableColor, 'important');
                      target.querySelectorAll('svg, svg *').forEach((icon) => {
                        if (!(icon instanceof SVGElement)) return;
                        const iconStyles = win.getComputedStyle(icon);
                        const stroke = iconStyles.stroke;
                        const fill = iconStyles.fill;

                        icon.style.setProperty('color', readableColor, 'important');
                        if (stroke && stroke !== 'none' && !stroke.includes('url(')) {
                          icon.style.setProperty('stroke', readableColor, 'important');
                        }
                        if (fill && fill !== 'none' && !fill.includes('url(')) {
                          icon.style.setProperty('fill', readableColor, 'important');
                        }
                      });
                    }
            ...
            syncHeaderFrame();
            window.setTimeout(syncHeaderFrame, 160);
          }}
        />
        );
      })()}
