...
const CARD_NAMES: Record<string, string> = { visa: 'Visa', mastercard: 'Mastercard', amex: 'American Express', bank_account: 'Bank Account' };

export function InsuranceSettings({ userName, userEmail, userPhone, accentColor, portalConfig, onTriggerVerification }: InsuranceSettingsProps) {
  ...
  const paymentMethods = config.insurancePaymentMethods || DEFAULT_INSURANCE_CONFIG.insurancePaymentMethods!;
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '700px', margin: '0 auto' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ fontSize: '14px', color: '#64748B', marginTop: '4px' }}>Manage your profile, payment, and policy preferences</p>
      </div>
      ...
      {/* Policy Holder Info */}
      <div style={sectionStyle}>
        <h3 style={sectionTitleStyle}><span>🛡️</span> Policyholder Details</h3>
        <p style={sectionDescStyle}>Your insurance profile</p>
        <div style={{ padding: '14px 0', borderBottom: '1px solid #F1F5F9' }}>
          <p style={{ fontSize: '13px', color: '#64748B', margin: 0 }}>Member Since</p>
          <p style={{ fontSize: '15px', color: '#0F172A', fontWeight: 500, margin: '2px 0 0' }}>{config.insurancePolicyHolderSince || '2019'}</p>
        </div>
        <div style={{ padding: '14px 0', borderBottom: '1px solid #F1F5F9' }}>
          <p style={{ fontSize: '13px', color: '#64748B', margin: 0 }}>Agent</p>
          <p style={{ fontSize: '15px', color: '#0F172A', fontWeight: 500, margin: '2px 0 0' }}>
            {config.insuranceAgentName || 'Michael Torres'} · {config.insuranceAgentPhone || '(555) 234-5678'}
          </p>
        </div>
        <SettingItem label="Beneficiary" value="Robert Cooper (Spouse)" accentColor={accentColor} sensitive
          onEdit={isTriggerEnabled('update your beneficiary') ? () => handleSensitiveAction('update your beneficiary') : undefined} />
      </div>
      ...
      <div style={sectionStyle}>
        ...
        <p style={sectionDescStyle}>Manage how you pay your premiums</p>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: '12px', color: '#94A3B8', margin: '2px 0 0' }}>
                  {pm.expiryDate ? `Expires ${pm.expiryDate}` : pm.detail || ''}
                </p>
              </div>
            </div>
