...
export function RetailSettings({ userName, userEmail, userPhone, accentColor, portalConfig, onTriggerVerification }: RetailSettingsProps) {
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '700px', margin: '0 auto' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ fontSize: '14px', color: '#64748B', marginTop: '4px' }}>Manage your profile, payment, and delivery preferences</p>
      </div>
      ...
      {/* Shipping Addresses */}
      <div style={sectionStyle}>
        <h3 style={sectionTitleStyle}><span>📦</span> Shipping Addresses</h3>
        <p style={sectionDescStyle}>Your saved delivery addresses</p>
        {addresses.map((addr, idx) => (
          <div key={idx} style={{
            display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between',
            padding: '14px 0', borderBottom: idx < addresses.length - 1 ? '1px solid #F1F5F9' : 'none',
          }}>
            <div>
              <p style={{ fontSize: '14px', fontWeight: 600, color: '#0F172A', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                {addr.label}
                {addr.isDefault && (
                  <span style={{ fontSize: '10px', fontWeight: 600, padding: '2px 8px', borderRadius: '4px', background: `${accentColor}15`, color: accentColor }}>DEFAULT</span>
                )}
              </p>
            ...
            <button onClick={() => { if (isTriggerEnabled('update your shipping address')) handleSensitiveAction('update your shipping address'); }} style={{
              padding: '6px 14px', fontSize: '13px', fontWeight: 500,
              color: accentColor, background: `${accentColor}10`,
              border: `1px solid ${accentColor}30`, borderRadius: '8px',
